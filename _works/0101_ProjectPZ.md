...
## 담당 업무
[절차적 레벨 생성](#절차적-레벨-생성){: .button .subject .arrow sub="Wang Tile 알고리즘을 응용한 절차적 지형 생성"}
[청크 시스템](#청크-시스템){: .button .subject .arrow sub="Quadtree를 사용한 청크 시스템"}
[CSV 유틸리티](#csv-유틸리티){: .button .subject .arrow sub="CSV 텍스트를 간편하게 직렬화하는 유틸리티"}
[커스텀 에디터](#커스텀-에디터){: .button .subject .arrow sub="맵 데이터 에디터와 프리뷰 기능"}
...
# 세부 내용
## 절차적 레벨 생성
[Wang Tile](https://en.wikipedia.org/wiki/Wang_tile) 알고리즘을 응용하여 절차적 지형을 생성하는 로직을 구현하였습니다.  
작은 지형 데이터(이하 영역)를 만들어 인접 가능한 지점과 조건을 정의하고, 해당 지점을 통해 영역이 회전-확장-연결되며 레벨이 생성됩니다.  
플레이 데이터에 따라 상호작용 오브젝트의 생성 여부와 위치가 변경되거나, 특정 영역이 포함되는 지형을 생성하도록 설정할 수 있었습니다.  
생성된 레벨 데이터를 기반으로 필요한 부분의 지형 메쉬를 생성하고, Tilemap 기능을 사용하여 인게임 월드맵을 구현하였습니다.

## 청크 시스템
**플레이어 위치 기준 최적화**: 플레이어의 위치 기준으로 영역 밖의 오브젝트 및 지형을 비활성화합니다.  
**타임 스탬프 기반 AI 동기화**: 플레이어가 청크 진입 시, 몬스터의 마지막 동작 시간과 현재 시간의 차이를 통해 몬스터의 행동을 연산 후 동기화합니다.  
플레이어가 위치한 지역을 기준으로 일정 범위 내의 지역만을 활성화합니다.  
