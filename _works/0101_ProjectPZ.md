---
layout: basic
title: "Project PZ"
description: "2021-2025"
image: /images/pic01.jpg
---
## 담당 업무

### 절차적 레벨·메쉬 생성
- 절차적 레벨 생성: [Wang Tile](https://en.wikipedia.org/wiki/Wang_tile) 알고리즘을 응용하여 절차적인 지형 생성 구현
- 절차적 메쉬 생성: 생성된 레벨 데이터를 기반으로 필요한 부분의 지형 메쉬를 생성·텍스처 적용
- Tilemap과 연동: 생성된 레벨 데이터를 Tilemap에 적용하여 인게임 월드맵 구현

### 몬스터 패턴 시스템
- 데이터 기반 몬스터 AI: 디자이너가 작성한 데이터에 따라 몬스터가 행동하는 패턴 시스템 구현

### 청크 시스템
- 청크 기반 최적화: 플레이어의 위치를 기준으로 청크 밖의 오브젝트는 비활성화하여 CPU 오버헤드 최소화
- 타임 스탬프 기반 AI 동기화: 플레이어가 청크 진입 시, 몬스터의 마지막 동작 시간과 현재 시간의 차이를 통해 몬스터의 행동을 연산 후 동기화

### 커스텀 에디터
- 레벨 디자인 툴: 절차적 생성을 위한 베이스 데이터 및 사전 정의된 레벨 작업 툴 구현
- 몬스터 패턴 디버깅 툴: 실시간 몬스터 패턴 추적 및 상태 확인용 툴 구현
- 데이터 기반 구조물 배치: 플레이 데이터에 따라 구조물의 종류와 위치를 동적으로 변경하는 시스템 구현

### CSV 파싱 자동화(?)
- 뭐라고 설명해야할지 애매함

### 성능 최적화
- 절차적 레벨 생성 프로세스 개선: 각종 최적화 기법을 적용하여 레벨 생성 시간을 대폭 감소
- UniTask 적용: 비동기 로직(로딩, 에셋 로드 등)에 UniTask를 적용하여 메모리 할당을 최소화


## 절차적 레벨 생성
미리 만들어진 작은 지형 데이터의 집합으로 영역을 만들고, 영역의 집합으로 하나의 레벨을 만들어내는 로직을 구현하였습니다.  
영역과 영역은 인접 가능한 지점이 정해져있고, 해당 지점을 통해서만 이동이 가능한 구조로 생성되어야 했음  
이는 절차적 텍스처 합성에 사용되는 Wang Tile을 응용하여 실현 가능하다고 판단, 생성 로직을 구상 후 문서로 작성하여 디자이너와 나의 뇌를 동기화.  

## 청크 시스템

## 맵 데이터 에디터·프리뷰 기능
결과적으로는 그림판 수준이지만 일단 기능 나열...  
- 포토샵 같은 곳에 있는 네비게이션 기능
- 브러쉬 크기 조절, 브러쉬 마스크 기능...
- 인게임 적용 예시 보기(?)
    - 지금까지의 데이터를 인게임처럼 보여준다. 인게임과 동일한 메시 생성기를 사용하므로 모듈화도 되어있다는 걸 자랑할 수 있다...  
    - 플레이어 조작도 가능...
- 에셋 프리뷰 쪽에서 시각화된 상태로 보여준다...
    - 팀원들의 빠른 보고서 작성을 위해 png로 출력해주는 기능도 있음...  
UIToolkit을 사용하여 기능과 디자인을 분리했으므로 나중에 UI 디자이너가 요소를 수정하기 편하게 했음...  

## 몬스터 패턴 디버깅 툴
몬스터가 어떤 틱에서 어떤 행동을 취했는지 목록으로 보여준다...  
디자이너가 작성한 데이터대로 굴러가는지 보는 용도였다...  

## 데이터 기반 몬스터 패턴 시스템
디자이너가 쓴 텍스트를 파싱해서 지정된 함수를 호출시키는 형태이다...  

## CSV 형식 파싱 자동화(?)
이거는 Attribute를 통해서...헤더의 이름과 동일한 변수에 값을 넣어주는 친구이다...  
List와 Dictionary 두가지 방식을 사용할 수 있고...Dictionary의 경우 키를 무슨 값으로 할건지 이름 정해줄 수 있음...  



# Stuck & Strike (Troubleshooting)

[Zero allocation과 Copy propagation를 통한 성능 개선](#){: .btn-sub sub="절차적 레벨 생성 과정에서의 문제 발생과 해결"}


### 문제 발생
아니 어느 순간부터 맵 한번 만들어지는거 보려면 무슨 8분 10분을 기다려야하더라고요?  
이게 뭐 특정 컴퓨터에서만 오래걸리는게 아니라 뭐 뭘 써도 다 똑같이 오래걸림  
나중에 서버코드로 옮겨야하는데 벌써부터 이렇게 오래걸리면 진짜 미래가 걱정되는 수준 아니겠음?...  

### 원인 파악
#### 간접적 원인 파악
1. 프로토타입 코드의 잔재: 재미검증용으로 작성된 프로토타입 코드를 리팩토링하지 않고 그대로 사용
2. 기술 부채 누적: 레벨 스펙 변경 시, 코어 로직을 수정하는 대신 결과물을 수정하여 출력하는 방식으로 대응하여 불필요한 연산이 제거되지 못함.

#### 직접적 원인 파악
Profiler 분석 결과 심각한 GC 오버헤드(GC.Alloc, GC.Collect)와 자주 호출되는 함수의 긴 동작 시간으로 인한 문제임을 파악하였음.  
이후 ProfilerMarker를 통해 어떤 부분에서 GC 오버헤드가 발생하는지, 또 함수의 어떤 부분이 동작 시간을 증가시키는지를 알아냈음.  

1. 불필요한 객체 생성으로 인한 GC 오버헤드
    - 좌표 클래스의 연산자 오버로딩으로 인해 매 연산마다 새로운 객체 생성
    - 다음 지형 생성 위치를 선정하기 위한 오버랩 체크 시 타일의 개수만큼 새로운 객체 생성
    - 결과물 가공 후 버려지는 데이터들의 처리
2. LINQ 사용으로 인한 GC 오버헤드
    - ToList, ToDictionary등의 함수로 인한 새로운 객체 생성 및 반복적인 Resizing
    - 콜렉션의 Enumerator가 LINQ용 인터페이스로 캐스팅
3. Coroutine 사용으로 인한 GC 오버헤드

### 문제 해결

#### CPU 처리 속도 개선
- 좌표 클래스를 좌표 구조체로 변경 후, 크기를 8byte로 줄여 CPU 처리 속도 개선
- 불필요한 중간 객체를 제거하고 함수 파라미터에 직접 계산식을 넣어 CPU 처리 속도 개선

#### 구조 재설계
- 지형 생성 위치 선정 로직 수정
    - 런타임 최적화에 사용하던 청크 구조를 도입하여 모든 과정의 평균 연산 속도 개선
    - List 순회 대신 HashSet을 사용하여 데이터 접근 속도 개선
- 결과물 가공 프로세스 제거, 되도록 필요한 데이터만 사용하도록 개선

#### GC 오버헤드 제거
우선 LINQ 쪽을 해결  
- 사용할 콜렉션은 미리 생성해두고, 생성 시 크기를 미리 예측하여 설정함
- for/while을 사용한 순회로 새로운 콜렉션을 생성하지 않도록 함

그리고 불필요한 객체 생성 해결
- 연산자 오버로딩을 제거하고 연산용 함수로 변경
- 좌표 클래스를 좌표 구조체로 변경하여 GC 해방
- 구조 재설계 과정에서 일부 지형 클래스의 생성을 단순 계산식으로 수정
