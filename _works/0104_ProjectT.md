---
layout: basic
title: "Project T"
description: "2D 퍼즐 플랫포머"
image: /images/pic01.jpg
---

## 프로젝트 개요
아 오케이 공백 두개로 끝내면 된다 이거지?  
오케이 오케이 이해 했음. 이제 되겠지?  

이거는 아예 문단이 나눠지고?

## 담당 업무
- 플레이어 컨트롤
- 스킬 시스템
- 대화 이벤트 기반 퀘스트 시스템
- 감정 기반 동적 NPC 보이스 시스템
- 퍼즐
- UI 시스템


## 담당 업무
- Github/SourceTree를 사용한 게임 버전 관리
- 플레이어
    - 이동, 점프, 사다리타기 등 기본 동작 구현
    - 오브젝트 들기, 퍼즐 상호작용, 스킬 등 특수 동작 구현
- 스킬 시스템
    - 물, 번개, 바람 스킬 기능 구현
    - 식물, 기계 속성 오브젝트와의 상호작용 기능 구현
- NPC
    - 대화 시 보이스 출력 구현
    - 대화를 통한 퀘스트 진행 구현
- 퀘스트 시스템
    - NPC와의 대화를 통한 수령/완수 구현
    - 대화 시스템을 통해 대화 시작, 대화 중, 대화 끝 세가지의 퀘스트 관련 검사 구현
- 대화 시스템
    - NPC와의 대화 기능 구현
    - CSV확장자의 대화 시스템 양식 제작
        - NPC 이름, 대사 출력
        - 각각의 보이스 출력
- UI 기능 구현
    - 인게임 UI 기능 전반 구현
        - 키보드만 사용해도 모든 UI 관련 조작이 가능하게끔 설계
        - 대화 시스템에 따른 대화 UI 출력
        - Json 입출력으로 환경설정 저장
        - 컷씬 만화 출력
        - 로딩화면
- 카메라
    - 플레이어 팔로우 구현
        - 플레이어 외의 다른 오브젝트 팔로우 구현
    - 박스 콜라이더를 사용한 카메라 영역 제한 구현
- 사운드
    - 부드러운 사운드 전환 구현
    - 배경음, 효과음, 환경음 적용
    - 대화 시 보이스 출력
        - NPC마다 개별적인 보이스 출력
        - 특정 글자마다 피치값을 수정하거나, 랜덤으로 출력
- 대화 스크립트 피드백
- Stage_00 레벨디자인

### 대화 이벤트 기반 퀘스트 시스템
NPC와의 대화 시작, 종료, 혹은 특정 대사 출력을 퀘스트의 완료나 실패 조건으로 설정할 수 있는  
이벤트 기반 퀘스트 시스템 구현

### 감정 기반 동적 보이스
NPC 대화 시 설정된 감정 상태에 따라 실시간으로 피치와 톤, 속도 등이 변조된 사운드를 출력하는 기능 구현

### 퍼즐 시스템
물, 번개, 바람 등 스킬 속성과 식물, 기계 속성 오브젝트 간의 상호작용을 통한 퍼즐 시스템 구현

### 기타
- UGUI Navigation: 키보드만으로 UI의 조작이 가능하도록 설계
- 카메라 포커스: 플레이어나 퍼즐 오브젝트 등, 특정 타겟 오브젝트로 포커스를 전환할 수 있는 카메라 로직 구현
- 대화 시스템: CSV 파일을 통해 NPC와의 대화 및 퀘스트 이벤트를 설정할 수 있는 시스템 구현
